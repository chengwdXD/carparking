...
const submitSearch = async () => {//按鈕送出事件
  ...
  await apiCarStore.cardata(query)
 // console.log('搜尋關鍵字:', query);
  ...
  // 查無資料提醒
  if (!apiCarStore.data?.result?.results?.length) {
    Swal.fire({
      title: '查無資料',
      confirmButtonText: '確定',
      confirmButtonColor: '#3490dc'
...
const pageview: number = 20 // 一頁最多 20 筆
...
const pageshowdata = computed(() => {
  const list = apiCarStore.data?.result?.results ?? []
...
// 當搜尋結果長度改變時，自動回到第一頁
watch(
  () => apiCarStore.data?.result?.results?.length,
  () => { currentpage.value = 1 }
)
</script>
...
<template>
   <div class="relative flex justify-center  min-h-screen bg-gray-100  bg-[url('/images/chargecar.png')] bg-[length:100%_100%] bg-no-repeat bg-center  ">
    <div class="absolute inset-0 bg-white/5 backdrop-blur-sm"></div>
     ...
     <div @click="resetSearch" class="font-mono cursor-pointer">臺北市電動車充電站資訊</div>
   <div  class="p-4 flex items-center space-x-2 mx-auto max-w-3xl flex-col md:flex-row">
      ...
      <div class="overflow-x-auto">
          ...
          <tbody v-if="pageshowdata.length > 0">
            <tr v-for="(item, key) in pageshowdata" :key="key" class="group">
              <td
                class="border border-gray-300 px-4 py-2 bg-white dark:bg-[rgb(51,71,94)] text-black dark:text-white group-hover:bg-gray-200 group-hover:dark:bg-[rgb(117,77,77)] hidden md:table-cell">
                {{ (safeCurrentPage - 1) * pageview + key + 1 }}
              </td>
              <td
                class="border border-gray-300 px-4 py-2 bg-white dark:bg-[rgb(51,71,94)] text-black dark:text-white group-hover:bg-gray-200 group-hover:dark:bg-[rgb(117,77,77)]">
                {{ item.廠商 }}
              </td>
              <td
                class="border border-gray-300 px-4 py-2 bg-white dark:bg-[rgb(51,71,94)] text-black dark:text-white group-hover:bg-gray-200 group-hover:dark:bg-[rgb(117,77,77)]">
              ...
              <td
                class="border border-gray-300 px-4 py-2 bg-white dark:bg-[rgb(51,71,94)] text-black dark:text-white group-hover:bg-gray-200 group-hover:dark:bg-[rgb(117,77,77)] max-w-xs">
                {{ item.地址 }}
              </td>
              <td
                class="border border-gray-300 px-4 py-2 bg-white dark:bg-[rgb(51,71,94)] text-black dark:text-white group-hover:bg-gray-200 group-hover:dark:bg-[rgb(117,77,77)]">
                <a :href="'https://www.google.com/maps/search/?api=1&query=' + item.地址" target="_blank"
      ...
      <!-- 沒資料 -->
      <div v-if="apiCarStore.data && totalpages === 0" class="text-center text-gray-500 py-4">
        查無資料
      </div>
      <div v-else-if="!apiCarStore.data" class="text-center text-gray-500 py-4">
        Loading...
      </div>

      <!-- 分頁控制 -->
      <div v-if="totalpages >= 1" class="flex justify-center mt-4">
        <button v-if="safeCurrentPage > 1" @click="gotopage(safeCurrentPage - 1)"
        ...
        <div class="hidden sm:flex items-center">
          <button v-for="page in totalpagesarray" :key="page" @click="gotopage(page)"
            :class="{ 'bg-blue-500 text-white option': safeCurrentPage === page, 'bg-gray-300 text-black': safeCurrentPage !== page }"
        ...
        <button v-if="safeCurrentPage < totalpages" @click="gotopage(safeCurrentPage + 1)"
